...
int main(int argc, char* argv[]) {
	...
	*/
	...
	PetscInt localRowPredict = PETSC_DECIDE, localColumnPredict = PETSC_DECIDE;
	PetscSplitOwnership(PETSC_COMM_WORLD, &localRowPredict, &globalRow);
	PetscSplitOwnership(PETSC_COMM_WORLD, &localColumnPredict, &globalColumn);
	/*获取Petsc的默认在本处理器上的分割方法，来获取对角块大小
	参数2和3可以有一个是PETSC_DECIDE，自动获取另一个数*/
	PetscSynchronizedPrintf(PETSC_COMM_WORLD, "The predict diagonal portion size of on processor %d is %d x %d\n", rank, localRowPredict, localColumnPredict);
	PetscSynchronizedFlush(PETSC_COMM_WORLD, PETSC_STDOUT);

	//创建矩阵步骤五（完）
	ierr = MatMPIAIJSetPreallocation(A, localColumnPredict, NULL, globalColumn - localColumnPredict, NULL);CHKERRQ(ierr);
	/*为矩阵分配内存空间，调用矩阵相应的函数
	参数2：每一行对角块上的非0元素数量（每一行都相同的情况）
	参数3：每一行对角块上的非0元素数量（每一行都不同的情况，所以传一个和行数一样大的数组）
	参数2和3二选一，当参数3为空则使用参数2，否则使用参数3
	参数4：每一行非对角块上的非0元素数量（每一行都相同的情况）
	参数5：每一行非对角块上的非0元素数量（每一行都不同的情况，所以传一个和行数一样大的数组）
	参数4和5二选一，当参数4为空则使用参数5，否则使用参数4*/
    ...
    if (rank == 0)
    	PetscPrintf(PETSC_COMM_WORLD, "Read matrix from file:\n");
